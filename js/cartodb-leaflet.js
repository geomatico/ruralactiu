...
if( typeof (L.CartoDBLayer) === "undefined") {
	...
	L.CartoDBLayer = function(params) {
		...
		if(this.params.auto_bound)
			autoBound(this.params);
		...
		if(this.params.infowindow) {
			...
			// Always add cartodb tiles, simple or with wax.
		}

		addCartodb(); this.params.visible = true, this.params.active = true;
			...
			reqwest({
				...
				success : function(result) {
					if(result.rows[0].st_extent != null) {
						...
						// Check bounds
						if(coor1[0] > 180 || coor1[0] < -180 || coor1[1] > 90 || coor1[1] < -90 || coor2[0] > 180 || coor2[0] < -180 || coor2[1] > 90 || coor2[1] < -90) {
							coor1[0] = '-30';
							coor1[1] = '-50';
							coor2[0] = '110';
							coor2[1] = '80';
		...
		function addSimpleCartoDBTiles(params) {
			...
			var tile_style = (params.tile_style) ? encodeURIComponent(params.tile_style.replace(/\{\{table_name\}\}/g, params.table_name)) : '', query = encodeURIComponent(params.query.replace(/\{\{table_name\}\}/g, params.table_name));

			// Add the cartodb tiles
			var cartodb_url = 'http://' + params.user_name + '.cartodb.com/tiles/' + params.table_name + '/{z}/{x}/{y}.png?sql=' + query + '&style=' + tile_style, cartodb_layer = new L.TileLayer(cartodb_url, {
				attribution : 'CartoDB'
			});
			...
			params.waxOptions = {
				callbacks : {
					...
					click : function(feature, div, op3, evt) {
						var container_point = params.map.mouseEventToLayerPoint(evt), latlng = params.map.layerPointToLatLng(container_point);
...
L.CartoDBInfowindow = L.Class.extend({
	...
	_close : function() {
		if(this._opened) {
			var div = this._container, that = this;
	...
	_update : function() {
		var that = this, infowindow_sql = 'SELECT * FROM ' + this.options.table_name + ' WHERE cartodb_id=' + this._feature;
	...
	_updateContent : function(variables) {
		...
		this._contentNode.innerHTML = this._contentHTML(variables);

		// Show cartodb-id
		//this._tipContainer.innerHTML = '<label>id: <strong>'+this._feature+'</strong></label>';
	},

	_contentHTML : function(variables) {
		if(variables === undefined)
			return "";

		console.log(variables);
		//console.log(variables.tipo_actividad);
		var image;
	...
	_adjustPan : function() {
		...
		var containerHeight = this._container.offsetHeight, layerPos = new L.Point(this._containerLeft, -containerHeight - this._containerBottom), containerPos = this._map.layerPointToContainerPoint(layerPos), adjustOffset = new L.Point(0, 0), padding = this.options.autoPanPadding, size = this._map.getSize();
 ...
 * license MIT
 */
! function(a, b) { typeof module != "undefined" ? module.exports = b() : typeof define == "function" && define.amd ? define(a, b) : this[a] = b()
}("reqwest", function() {
	function handleReadyState(a, b, c) {
		return function() {a && a[readyState] == 4 && (twoHundo.test(a.status) ? b(a) : c(a))
		}
	}
	...
	function serializeHash() {
		var a = {};
		return eachFormElement.apply(function(b, c) { b in a ? (a[b] && !isArray(a[b]) && (a[b] = [a[b]]), a[b].push(c)) : a[b] = c
		}, arguments), a
	}

	var context = this, win = window, doc = document, old = context.reqwest, twoHundo = /^20\d$/, byTag = "getElementsByTagName", readyState = "readyState", contentType = "Content-Type", requestedWith = "X-Requested-With", head = doc[byTag]("head")[0], uniqid = 0, lastValue, xmlHttpRequest = "XMLHttpRequest", isArray = typeof Array.isArray == "function" ? Array.isArray : function(a) {
	...
	return Reqwest.prototype = {
		...
		return a && (a.type && (a.method = a.type) &&
		delete a.type, a.dataType && (a.type = a.dataType), a.jsonpCallback && (a.jsonpCallbackName = a.jsonpCallback) &&
		delete a.jsonpCallback, a.jsonp && (a.jsonpCallback = a.jsonp)), new Reqwest(a, b)
	}, reqwest
})
/*!
 ...
 */! function(a) {
	function A(a, b) { a = typeof a == "string" ? document.getElementById(a) : a, b = z(b);
		var c = {
			duration : b.duration,
			easing : b.easing,
			after : b.after
	...
	function z(a) {
		var b = {};
		for(var c in a)b[c] = a[c], c == "after" &&
		delete a[c];
		return b
	}

	function y(a, b, c) {
		var d = [], f = [], i = c.duration || 1e3, j = c.easing || "ease-out", k = ""; i = i + "ms", a.addEventListener(l, function m() {a.setAttribute("style", k), c.after && c.after(), a.removeEventListener(l, m, !0)
	...
	function x(a, b, c, d) {
		c = c || {};
		var e = w(b), f = a.currentStyle ? a.currentStyle : getComputedStyle(a, null), g = {}, h = +(new Date), i, j = c.duration || 200, k = h + j, l, m = c.easing ||
		function(a) {
		...
		l = setInterval(function() {
			...
			for(f in e)
			a.style[f] = e[f].f(g[f].v, e[f].v, m(i)) + e[f].u;
	...
	function u(a, b, c) {
		...
		if(t(f, 0) == "r") {
			...
			while(e--)
			h.push(~~f[e])
		} else {f.length == 4 && ( f = "#" + t(f, 1) + t(f, 1) + t(f, 2) + t(f, 2) + t(f, 3) + t(f, 3));
			while(e--)
			h.push(parseInt(t(f, 1 + e * 2, 2), 16))
		}
	...
	for(var m = j.split(" "), n = m.length; n--; )
	h[m[n]] = 1;
	var B = a.emile;
